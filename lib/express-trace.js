...
function trace(app) {
  ...
  for (var i = 0; i < len; ++i) {
    stack[i].handle = (function(route, fn){
      // TODO: route.routes patch to express
        ...
        for (var method in fn.routes) {
          fn.routes[method].forEach(function(route){
            ...
            route.callback = function(req, res, next){
              console.error('  \033[90m%s %s\033[0m', route.method, route.path);
  ...
  stack.unshift({
    ...
    , handle: function(req, res, next){
      ...
      res.on('finish', function(){
        ...
        console.error('\n  \033[90mresponded to %s\033[0m \033[33m%s\033[0m \033[90min %dms with \033[0m\033[' + color + 'm%s\033[0m'
          , req.method
          , req.url
          , new Date - start
          , res.statusCode);
